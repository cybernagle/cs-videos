...
class TechPart(Scene):
    ...
    def construct(self):
        ...
        # 磁盘的利用率有所增加, 但是并没有很高, 从原有的 70%, 上升到 75% 左右.并保持稳定
        ...
        ram = self.create_memory(
            5, color=YELLOW, width=0.5, height=1.5,x_grid=0.5, y_grid=0.5
        )
        self.add(ram)
        ...
        disk_arrangement = Rectangle(width=4.0, height=2.0, fill_opacity=0.5, color=GREEN).set_fill(color=GREEN)
